...
void ltdc_draw_font(uint16_t x, uint16_t y, uint8_t chars, uint8_t size) {

    uint8_t num = (uint8_t) (chars - ' ');
    for (uint8_t i = 0; i < 12; ++i) {
        uint8_t tmp = Font12_Table[num * 12 + i];
...
void ltdc_draw_string(uint16_t x, uint16_t y, uint8_t *s) {
